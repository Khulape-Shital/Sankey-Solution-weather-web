...
function displayFavoriteCities() {
    ...
    const favoriteCitiesSection = document.getElementById('favorite-cities');
    const favoriteCitiesBox = document.getElementById('favorite-cities-box');
    
    if (favorites.length === 0) {
        favoriteCitiesSection.classList.add('hidden');
        return;
    }
    
    favoriteCitiesSection.classList.remove('hidden');
    
    favoriteCitiesBox.innerHTML = favorites.slice(0, 6).map(favorite => `
        <div class="favorite-city-item" onclick="redirectToCity('${favorite.name}')">
            <div class="city-name-fav">${favorite.name}</div>
            <div class="city-temp-fav">${favorite.temp}°</div>
        </div>
    `).join('');
}

function redirectToCity(cityName) {
    // Store the city name and redirect to search page
